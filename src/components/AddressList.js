...
const AddressList = () => {
  ...
  const handleUpdate = (event) => {
    ...
    axios.put(`http://localhost:5000/api/addresses/${selectedAddress._id}`, selectedAddress)
      .then(() => {
        axios.get('http://localhost:5000/api/addresses').then((response) => {
          setAddresses(response.data);
          ...
          <form onSubmit={handleUpdate}>
            ...
            <div className="form-group">
              ...
              <input
                ...
                onChange={(e) => setSelectedAddress({ ...selectedAddress, contact: e.target.value })}
                required
              />
            </div>
            <div className="form-group">
      ...
      <ul>
        {addresses.map((address) => (
          <div className='container border border-dark rounded m-2 p-2 text-right' key={address._id}>
            ...
            <button className="btn btn-sm" onClick={() => handleDelete(address._id)}>
              <h5><Trash /></h5>
            </button>
            <button type="button" className="btn" onClick={() => handleEdit(address)}>
              <h5><PencilSquare /></h5>
            </button>
          </div>
        ))}
